...
% state. The fidelity is returned, along with its gradient
% with respect to amplitudes of all control operators at every time step
% of the shaped pulse. Syntax:
%
%  [fwd_traj,fidelity,grad]=grape_approx(spin_system,drifts,controls, waveform,rho_init,rho_targ,fidelity_type, order)
%
% Parameters:
%
...
%
% Note: the step_approx function computes the step propagator with an approximated Taylor series.
%
% Modified by mengjia.he@kit.edu, 2026.01.27

function [fwd_traj,fidelity,grad]=grape_approx(spin_system,drifts,controls,...
                                              waveform,rho_init,rho_targ,...
                                              fidelity_type, order) %#ok<*PFBNS>
% Count the outputs
n_outputs=nargout();
...
switch spin_system.control.integrator
    ...
    case 'rectangle'
        ...
        for n=1:nsteps
            ...
            fwd_traj(:,n+1)=step_approx(spin_system,L_forw,fwd_traj(:,n),+dt(n),order);
            bwd_traj(:,n+1)=step_approx(spin_system,L_back,bwd_traj(:,n),-dt(nsteps+1-n),order);
    ...
    case 'trapezium'
        ...
        for n=1:nsteps
            ...
            % Take a time step forwards and backwards
            fwd_traj(:,n+1)=step_approx(spin_system,{ L_forw_left,...
                                              (L_forw_left+L_forw_right)/2,...
                                               L_forw_right},fwd_traj(:,n),+dt(n),order);
            bwd_traj(:,n+1)=step_approx(spin_system,{ L_back_right,...
                                              (L_back_right+L_back_left)/2,...
                                               L_back_left},bwd_traj(:,n),-dt(nsteps+1-n),order);
        ...
        case 'rectangle'
            ...
            for n=1:nsteps
                ...
                for k=1:nctrls
                    ...
                    aux_vec=step_approx(ss_parfor,aux_matrix,aux_vec,dt(n),order);
            ...
            for n=1:(nsteps+1)
                ...
                if n==1
                    ...
                    for k=1:nctrls
                        ...
                        [DL_first,~]=aux_mat(L,controls,dt(n),waveform(:,1),waveform(:,2),k);

                        % Build the auxiliary vector
                        aux_vec=[zero_state; fwd_traj(:,n)];

                        % Propagate the auxiliary vector
                        aux_vec=step_approx(ss_parfor,DL_first,aux_vec,dt(n),order);
                ...
                elseif n==(nsteps+1)
                    ...
                    for k=1:nctrls
                        ...
                        % Build the auxiliary matrix
                        [~,DR_last]=aux_mat(L,controls,dt(n-1),waveform(:,(end-1)),waveform(:,end),k);
                        
                        % Build the auxiliary vector
                        aux_vec=[zero_state; fwd_traj(:,n-1)];

                        % Propagate the auxiliary vector
                        aux_vec=step_approx(ss_parfor,DR_last,aux_vec,dt(n-1),order);
                ...
                % Middle steps
                    ...
                    for k=1:nctrls
                        ...
                        % Auxiliary matrix
                        [Right_DL,~]=aux_mat(L,controls,dt(n),waveform(:,n),waveform(:,n+1),k);

                        % Build the auxiliary vector
                        aux_vec_a=[zero_state; fwd_traj(:,n)];

                        % Propagate and extract derivative action
                        aux_vec_a=step_approx(ss_parfor,Right_DL,aux_vec_a,dt(n),order); 
                        ...
                        [~,Left_DR]=aux_mat(L,controls,dt(n-1),waveform(:,n-1),waveform(:,n),k);

                        % Build the auxiliary vector
                        aux_vec_b=[zero_state; fwd_traj(:,n-1)];
                        
                        % Propagate and extract derivative action
                        aux_vec_b=step_approx(ss_parfor,Left_DR,aux_vec_b,dt(n),order);
