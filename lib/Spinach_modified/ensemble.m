...
n_power_levls=numel(spin_system.control.pwr_levels{1});	% Power level count
...
%% Edited by Mengjia He, 2023.03.23   %%
% Ensemble correlation: own state pair for each control power
if ismember('rho_power',spin_system.control.ens_corrs)
    catalog(catalog(:,1)~=catalog(:,3),:)=[];
    n_state_pairs=1;
end

% Ensemble correlation: own state pair for each control power for each
% drift
if ismember('rho_power_drift',spin_system.control.ens_corrs)
    catalog(catalog(:,1)~=catalog(:,3),:)=[];
    catalog(catalog(:,3)~=catalog(:,2),:)=[];
    n_state_pairs=1;
    n_power_levls=1;
...
% for n=1:n_cases
    ...
    % Get the power_level
    % edited by Mengjia He, transform () to {}
    % power_lvl=spin_system.control.pwr_levels{n_pwr};

    %% Edited by Mengjia He, 2023.03.23   %%
    % site 2: expand to multiple rows of pwr_levels,
    % corresponding to different transmit channel
    % Apply field levels to the waveform
    % if ~isempty(spin_system.control.field_levels)

    % % put the pwr_levels to unit
    % power_lvl = 1;

    % % Get the field value
    % field=spin_system.control.field_levels(:,n_pwr);

    % % Apply the phase
    % field = repmat(field,1,size(local_waveform,2));
    % field = kron(field, ones(2, 1));
    % local_waveform = local_waveform .* field;

    % end

    %% Edited by Mengjia He, 2025.03.10  %%
    % transform field_levels back to power levels for uniform usuage

    for  k=1:(size(local_waveform,1)/2)

        local_waveform(2*k-1:2*k,:) = spin_system.control.pwr_levels{k}(n_pwr) * local_waveform(2*k-1:2*k,:);


    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    %% Mengjia He, 2023.05.17  %%
    % add gradient field for RF pulse optimization in  controls
    % Apply field levels to the waveform
    % if ~isempty(spin_system.control.gradient_levels)

    % % Get the gradient field coefficients
    % gradField=spin_system.control.gradient_levels(:,n_pwr);
    % numRows = size(spin_system.control.gradient_levels,1);

    % % multiply the pulse current by the coefficients
    % gradField = repmat(gradField,1,size(local_waveform,2));
    % local_waveform(end-numRows+1:end,:) = local_waveform(end-numRows+1:end,:) .* gradField/power_lvl;
    % % clearvars numRows
    % end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    %% Edited by Mengjia He, 2022.03.17   %%
    % Get the phase_level
    if ~isempty(spin_system.control.phase_levels)

        % Apply phases levels to the waveform
        for k=1:(size(local_waveform,1)/2)
            ...
            % Get the phase
            phi=spin_system.control.phase_levels(n_pwr);
	...
	% Get drifts from pool ValueStore
	% Edited by Mengjia He, 2025.11.28, added option for non parallel pool
	if numWokers >0;  % parallel pool enabled
		if nworkers==0
			store=gcp('nocreate').ValueStore;
			L=store(['oc_drift_' num2str(n_sys)]);
		else
	...
	else		% parallel pool disabled
		L = spin_system.control.drifts{n_sys};
    ...
    if ~isempty(off_ens_sizes)
        ...
        for k=1:numel(off_ens_sizes)
            ...
            L = L + sparse(2*pi*spin_system.control.offsets{oper_idx}(vj)*spin_system.control.off_ops{oper_idx});
    ...
    if n_outputs==2
		...
		if strcmp(spin_system.control.type, 'ppt')

			% Fidelity and trajectory
			[traj_data{n},fidelities{n}]=grape(spin_system,L,spin_system.control.operators,...
				local_waveform,rho_init,rho_targ,spin_system.control.fidelity);
				
			
		elseif strcmp(spin_system.control.type, 'gate')
    ...
    elseif n_outputs==3
		...
		if strcmp(spin_system.control.type, 'ppt')
			...
			[traj_data{n},fidelities{n},gradients{n}]=grape(spin_system,L,spin_system.control.operators,...
				local_waveform,rho_init,rho_targ,spin_system.control.fidelity);
				
			% temporily change to approximated method, auxmat with second order Taylor approximation
			% [traj_data{n},fidelities{n},gradients{n}]=grape_approx(spin_system,L,spin_system.control.operators,...
				% local_waveform,rho_init,rho_targ,spin_system.control.fidelity,2);
				
			% temporily change to approximated method, second-order finite difference with second order Taylor approximation
			% [traj_data{n},fidelities{n},gradients{n}]=finiteDiff_approx(spin_system,L,spin_system.control.operators,...
				% local_waveform,rho_init,rho_targ,spin_system.control.fidelity,2);
    ...
    elseif n_outputs==4
		...
		if strcmp(spin_system.control.type, 'ppt')
			...
			[traj_data{n},fidelities{n},gradients{n},hessians{n}]=grape(spin_system,L,spin_system.control.operators,...
				local_waveform,rho_init,rho_targ,spin_system.control.fidelity);
				
		elseif strcmp(spin_system.control.type, 'gate')
		
			% Fidelity, gradient, Hessian and trajectory
			[traj_data{n},fidelities{n},gradients{n},hessians{n}]=grape_hilb_hessian(spin_system,L,...
				spin_system.control.operators,local_waveform,rho_init,rho_targ,spin_system.control.fidelity);
    ...
    if n_outputs>2
        ...
        %% Edited by Mengjia He, 2024.02.29 %%
        % if ~isempty(spin_system.control.field_levels)

        % gradients{n} = gradients{n} .* field;
        ...
        for k = 1:(size(gradients{n},1)/2)
            gradients{n}(2*k-1:2*k,:) = spin_system.control.pwr_levels{k}(n_pwr)*...
                gradients{n}(2*k-1:2*k,:);
        end
        % end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        gradients{n}=gradients{n}(:);
    ...
    if (n_outputs>3)&&strcmp(spin_system.control.integrator,'rectangle')
        ...
        %% Edited by Mengjia He, 2024.02.29 %%
        power_lvl= cellfun(@(x) x(n_pwr), spin_system.control.pwr_levels);
        power_lvl = transpose(power_lvl);
        power_lvl = repmat(power_lvl,1,size(local_waveform,2));
        power_lvl = kron(power_lvl, ones(2, 1));

        %% Edited by Mengjia He, 2024.02.29 %%
        % % if ~isempty(spin_system.control.field_levels)

        % field_matrix = power_lvl(:) * transpose(power_lvl(:));

        % hessians{n} = hessians{n} .* field_matrix;

        % % else
        power_lvl_matrix = power_lvl(:) * transpose(power_lvl(:));
        hessians{n} = hessians{n} .* power_lvl_matrix;

        % end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        hessians{n}=hessians{n}(:);
    end
...
if ~isempty(spin_system.control.plotting)
    ...
    % Reshape the fidelities for plotting
    fidelities=reshape(fidelities,n_state_pairs,n_ens_systems,...
        n_power_levls,n_offset_vals,...
        n_phase_specs);
