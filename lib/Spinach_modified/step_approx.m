...
% anu.acharya@soton.ac.uk
%
% Note: this function was adapted based on a Spinach function: <https://spindynamics.org/wiki/index.php?title=step.m>
% Note: this function provides an approximated step propagation, the Taylor series is truncated at the given order.

function rho=step_approx(spin_system,L,rho,time_step,order)
...
% if parfor_makes_sense
    ...
    % parfor m=1:size(rho,2)
        ...
        % rho(:,m)=reordered_taylor(L,rho(:,m),time_step,nsteps,order);
...
% else
    ...
    % Call Taylor-times-vector procedure
    rho=reordered_taylor(L,rho,time_step,nsteps,order);
...
function rho=reordered_taylor(L,rho,t,nsteps,order)
...
% if (~isnumeric(rho))&&(~iscell(rho))
    % error('rho must be a matrix, a vector, or a cell array thereof');
% end
% end
