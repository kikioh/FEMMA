...
% of the shaped pulse. 
%
% Using second-order finite difference to compute the gradient of step 
...
%
% Note: the step_approx function provided a approximated Taylor series.
%
% Modified by mengjia.he@kit.edu, 2026.01.27

function [fwd_traj,fidelity,grad]=finiteDiff_approx(spin_system,drifts,controls,waveform,rho_init,rho_targ,fidelity_type,order) %#ok<*PFBNS>
        ...
        case 'rectangle'
            ...
            for n=1:nsteps
                ...
                for k=1:nctrls
                    ...
                    aux_vec_fwd=step_approx(ss_parfor,L_fwd,aux_vec,dt(n),order);
					aux_vec_bwd=step_approx(ss_parfor,L_bwd,aux_vec,dt(n),order);
            ...
            for n=1:(nsteps+1)
                ...
                if n==1
                    ...
                    for k=1:nctrls
                        ...
                        % Propagate the auxiliary vector
                        aux_vec_fwd=step_approx(ss_parfor,G_fwd_l,aux_vec,dt(n),order);
						aux_vec_bwd=step_approx(ss_parfor,G_bwd_l,aux_vec,dt(n),order);
                ...
                elseif n==(nsteps+1)
                    ...
                    for k=1:nctrls
                        ...
                        % Propagate the auxiliary vector
                        aux_vec_fwd=step_approx(ss_parfor,G_fwd_r,aux_vec,dt(n-1),order);
						aux_vec_bwd=step_approx(ss_parfor,G_bwd_r,aux_vec,dt(n-1),order);
                ...
                % Middle steps
                    ...
                    for k=1:nctrls
						...
						% Propagate the auxiliary vector
                        aux_vec_fwd=step_approx(ss_parfor,G_fwd_l,aux_vec_a,dt(n),order);
						aux_vec_bwd=step_approx(ss_parfor,G_bwd_l,aux_vec_a,dt(n),order);
                        ...
                        % Propagate the auxiliary vector
                        aux_vec_fwd=step_approx(ss_parfor,G_fwd_r,aux_vec_b,dt(n-1),order);
						aux_vec_bwd=step_approx(ss_parfor,G_bwd_r,aux_vec_b,dt(n-1),order);
