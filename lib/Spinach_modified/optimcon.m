...
if isfield(control,'type')

    % Input validation
    if ~ischar(control.type)
        error('control.type must be a character string.');
    end
    
    % Absorb the input
    spin_system.control.type=control.type;
    control=rmfield(control,'type');

else

    % Default is point to point transfer
    spin_system.control.type='ppt';
...
if isfield(control,'operators')
    ...
    % Precompute control-control commutators
...
if isfield(control,'pwr_levels')
    ...
    report(spin_system,[pad('Control power ensemble size',60) ...
                        int2str(numel(spin_system.control.pwr_levels{1}))]);
    report(spin_system,[pad('   Max power multiplier at channel 1, Hz',60) ...
                        num2str(max(spin_system.control.pwr_levels{1})/(2*pi),'%.9g')]);
    report(spin_system,[pad('   Min power multiplier at channel 1, Hz',60) ...
                        num2str(min(spin_system.control.pwr_levels{1})/(2*pi),'%.9g')]);
...
% Absorb phase levels
if isfield(control,'phase_levels')
    
    % Absorb phase cycle specification
    spin_system.control.phase_levels=control.phase_levels;
    control=rmfield(control,'phase_levels');   
    
else
    
    % Empty array with one row
    spin_system.control.phase_levels=zeros(1,0);
    
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Edited by Mengjia He, 2022.03.17     %%
% Absorb gradient levels
if isfield(control,'gradient_levels')
    
    % Absorb gradient field level specification
    spin_system.control.gradient_levels=control.gradient_levels;
    control=rmfield(control,'gradient_levels');   
    
else
    
    % Empty array with one row
    spin_system.control.gradient_levels=zeros(1,0);
    
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Edited by Mengjia He, 2024.02.27    %%
% Absorb field levels
if isfield(control,'field_levels')

    
    % Absorb field level specification
    spin_system.control.field_levels=control.field_levels;
    control=rmfield(control,'field_levels');   
    
else
    
    % Empty array with one row
    spin_system.control.field_levels=zeros(1,0);
...
if isfield(control,'method')
    ...
    % Input validation
    if (~ischar(control.method))||(~ismember(control.method,{'lbfgs','newton','goodwin','mma','gcmma','mma-gcmma'}))
        error('control.method must be ''lbfgs'', ''newton'', or ''goodwin'', or ''mma'', or ''gcmma'', or ''mma-gcmma''.');
    end
...
%%% edit by Mengjia He, 2024.01.02
% Termination tolerance on the fidelity
if isfield(control,'tol_f')
    
    % Absorb the spec
    spin_system.control.tol_f=control.tol_f;
    control=rmfield(control,'tol_f');
    
else
    
    % Default is 0.9995
    spin_system.control.tol_f=0.9995;

end

% Inform the user
report(spin_system,[pad('Termination tolerance on fidelity',60) ...
                    pad(num2str(spin_system.control.tol_f,'%0.8g'),20)]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(control,'parallel')
	...
	%% Default setting changed by Mengjia He to 'ensemble', 2025.10.17
	spin_system.control.parallel='ensemble';
