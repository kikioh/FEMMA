...
numPenalty = numel(control.penalties);
switch control.objective
    ...
    case 'least-square'           % least-square objective of the overlap, each fidelity cooresponds to 2 constraints
        p = control.n_cases;
        mma.m = 2*p+numPenalty;
        mma.a0 = 1;
        mma.a = zeros(mma.m,1);
        mma.c = [zeros(2*p,1);1000 * ones(numPenalty,1)];
        mma.d = [2*ones(2*p,1);zeros(numPenalty,1)];
	...
	otherwise
		error('unknown objective type, try least-square in mma method');
